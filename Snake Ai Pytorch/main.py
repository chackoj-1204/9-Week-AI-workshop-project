...
class Field:
    ...
    def update_field(self):
        try:
            ...
            for part in self.Snake.fullSnek:
    ...
    def run(self, action):
        ...
        self.num_moves += 1
        reward = 0  # -1*(100 - ((self.Snake.x - self.Dot.x) ** 2 + (self.Snake.y - self.Dot.y) ** 2) ** .5) ** 2
        ...
        self.update_field()
        if ((30 <= self.Snake.x or 0 > self.Snake.x) or (
                30 <= self.Snake.y or 0 > self.Snake.y)) or 2 in self.body or self.num_moves > 100 * len(
                self.Snake.fullSnek):
...
class Agent():
    def __init__(self):
        ...
        self.model = self.DenseNet(11, 256, 3)
    ...
    def get_state(self, game):
        ...
        down = game.Snake.actions[game.Snake.direction] == [0, -1]
    ...
    def train_step(self, state, action, reward, next_state, done):
        ...
        for ind in range(len(action)):
            Q_new = reward[ind]
            if not done[ind]:
                Q_new = Q_new + self.gamma * torch.max(self.model(next_state[ind].reshape((1, 11))))
            targets[ind][torch.argmax(action[ind]).item()] = Q_new
        self.optimizer.zero_grad()
        loss = self.criterion(targets, pred)
        # writer.add_scalar("Loss/train", loss, self.num_preds)
        # self.num_preds += 1
        loss.backward()
    ...
    def get_action(self, past):
        ...
        if random.randint(0, 200) < self.epsilsion:
            ...
            past = torch.tensor(past, dtype=torch.float).reshape((1, 11))
